...
export class Patch {
    ...
    generatePatchEntries(patchesOwner, overrides = undefined) {
        ...
        Reflect.ownKeys(patchesOwner).forEach(key => {
            ...
            try {
                ...
                this.patchEntries[key] = new PatchEntry(key, patchesOwner, condition, overrides);
                this.patchCount += 1;
            }
            catch (error) {
     ...
     */
    get prettyEntries() {
        const prettyEntries = this.entries.map(([key, value]) => Patch.stringRef(Patch.extractName(key), key, value));
